    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('buildwatch', () => ({
                data: {
                    formeMontres: @json($formes),
                    arrierePlanMontres: @json($arrierePlans),
                    arrierePlanImages: [],
                    index: [],
                    indexImages: [],
                ...
                watch(){
                    ...
                    //     this.changerPrevisualisationArrierePlan(value)
                    // })
                },
                init(){
                    // alert(this.editMode)
                ...
                resetFormulaire(){
                    ...
                    document.querySelector('#selectIndex').selectedIndex = 0
                    document.querySelector('#selectIndexImage').selectedIndex = 0
                    this.data.indexImages = []
                    this.data.form.indexImage = {}


                    this.data.index       = []
                    this.data.form.index      = {}

                    this.data.form.forme      = {}
                },
                choisirForme(event){
                    // this.resetFormulaire()
                    // return
                    // alert(event.target.value)
                    const forme = this.data.formeMontres.find(item => item.id_forme == event.target.value)
                    ...
                    // console.log(forme.index);

                    this.data.form.forme = forme
                    this.data.index = forme.index
                },
                choisirArrierePlan(event){
                    ...
                    this.data.arrierePlanImages = arrierePlan.images

                    // console.log(arrierePlan.images);

                    // this.changerArrierePlan(arrierePlan.image_arriere_plan)
                },
                choisirArrierePlanImage(event){
                    const arrierePlanImage = this.data.arrierePlanImages.find(item => item.id_arriere_plan_media == event.target.value)

                    if (!arrierePlanImage) return

                    this.changerArrierePlan(arrierePlanImage.chemin)


                    console.log(arrierePlanImage);
                },
                changerArrierePlan(url){
                    ...
                    this.changerPrevisualisationArrierePlan(url)
                },
                choisirIndex(event){
                    const index = this.data.index.find(item => item.id_index == event.target.value)
                    ...
                    this.data.indexImages = index.images

                },
                choisirIndexImage(event){
                    const indexImage = this.data.indexImages.find(item => item.id_index_media == event.target.value)
                    ...
                    this.data.form.indexImage = indexImage
                },
                choisirImage(event){
...
<div class="col-md-8">
    <div class="card" x-show='data.etape == 1'>
        <div class="card-body">
            ...
            <p x-text="JSON.stringify(data.form.forme.nom)"></p>
            <p x-text="JSON.stringify(data.form.index.nom)"></p>
            <p x-text="JSON.stringify(data.index.length)"></p>
            <p x-text="JSON.stringify(data.indexImages.length)"></p>


            @if(isset($watch) && $watch->getKey())
            <form @submit.prevent="enregistrerMontre" action="{{ route('user.dashboard.watch.update', $watch) }}" method="post" id="enregisterMontreClient" x-ref="enregisterMontreClient">
                @method('PUT')
                @else
                            ...
                            <div class="mb-3">
                                ...
                                <select name="id_forme_montre" id="formeMontre" class="form-control" @change="choisirForme">
                                    ...
                                    <template x-for="formeMontre in data.formeMontres">
                                        <option :value="formeMontre.id_forme" x-text="formeMontre.nom"></option>
                                    </template>
                                </select>
                            </div>
                        </div>
                        <div class="col" x-show="!jQuery.isEmptyObject(data.index)" x-transition>
                            <div class="mb-3">
                                <label for="">index</label>
                                <select name="id_couleur_index" id="selectIndex" class="form-control" @change="choisirIndex">
                                    <option value="0">-------</option>
                                    <template x-for="index in data.index">
                                        <option :value="index.id_index" x-text="index.nom"></option>
                                    </template>
                                </select>
                            </div>
                        </div>
                        <div class="col" x-show="!jQuery.isEmptyObject(data.indexImages)" x-transition>
                            <div class="mb-3">
                                <label for="">index image</label>
                                <select id="selectIndexImage" class="form-control" @change="choisirIndexImage">
                                    <option value="0">-------</option>
                                    <template x-for="indexImage in data.indexImages">
                                        <option :value="indexImage.id_index_media" x-text="indexImage.nom"></option>
                                    </template>
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="id_arriere_plan" class="form-control" @change="choisirArrierePlan">
                                        <template x-for="arrierePlanMontre in data.arrierePlanMontres">
                                            <option :value="arrierePlanMontre.id_arriere_plan" x-text="arrierePlanMontre.nom"></option>
                                        </template>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6" x-show="!jQuery.isEmptyObject(data.arrierePlanImages)">
                                <div class="mb-3">
                                    <label for="">image arrière Plan</label>
                                    <select class="form-control" @change="choisirArrierePlanImage">
                                        <option value="0">-------</option>
                                        <template x-for="arrierePlanImage in data.arrierePlanImages">
...
<div class="col-md-4">
    <div class="row">
        <div class="col-12 text-center">
            <div class="card">
                <div class="card-body align-self-center" style="height: 300px;">
                    ...
                    <div style="position: relative;   width: 250px;background-position: center;
                    background-size: 250px; overflow: hidden" x-ref="previsualisation">
                        <img :src="data.form.forme.chemin" x-show="!jQuery.isEmptyObject(data.form.forme)"  clas="img-fluid" " style="width: 250px;" id="img_forme_montre">
                        <img :src="data.form.indexImage.chemin" x-show="!jQuery.isEmptyObject(data.form.indexImage)"
                        style="width: 193px; position: absolute;left: 22px; top: 42px;" id="img_index" >

                        <p x-ref="textContenu"
                            x-text='data.form.texte.contenu'
                            style="width: 80%; position: absolute;left: 10%;  top:45%; font-weight: 700; font-size: 20px;"></p>
                        {{-- <img src="arriere_plan_image" style="width: 70%; position: absolute;left: 50px; top: 45px;" id="img_index" > --}}
                    </div>

                </div>
                {{--
                    Faire un deuxieme pour previsualiser le carré
                    --}}
        ...
        {{-- <div class="col-12 text-center">
            <div class="card">
                <div class="card-body align-self-center" style="height: 300px;">
                    <div style="position: relative;   width: 300px;background-position: center;
                    background-size: 300px; overflow: hidden" x-ref="previsualisation">
                        <img src="http://localhost:8000/uploads/forme_montre/20221027134147.png" clas="img-fluid" alst="forme" style="width: 300px; positcion: relative" id="img_forme_montre">
                        <img src="/uploads/couleur_index/carre3.png" style="width: 164px; position: absolute;left: 68px; top: 50px;" id="img_index" >
                    ...
                    <p class="px-2 mt-3" style="background-color: rgb(149, 0, 255); height: 60px;">
